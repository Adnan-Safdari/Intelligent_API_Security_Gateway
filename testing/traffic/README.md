...
The collection output records deterministic detector evidence and reports only
attack-minutes that the gateway neither detected nor already refused, so a
policy/reflex refusal is never misreported as a detector miss.
...
suspicious. Without them an offline analysis would treat any non-zero signal as
an attack.
