...
func traversalHandler() http.Handler {
	return NewTraversalEnumDetector(DefaultTraversalEnumConfig()).Middleware(okBackend())
}
...
func TestTraversalDisabledInspectsNothing(t *testing.T) {
	handler := NewTraversalEnumDetector(TraversalEnumDetectorConfig{Enabled: false}).
...
func TestTraversalEmptyPatternsFallBackToDefaults(t *testing.T) {
	handler := NewTraversalEnumDetector(TraversalEnumDetectorConfig{Enabled: true}).
		Middleware(okBackend())
