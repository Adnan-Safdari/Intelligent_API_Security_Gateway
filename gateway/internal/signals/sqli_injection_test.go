...
// Documents a real gap: only the body is inspected, so injection through the
// query string is invisible. This test asserts current behaviour -- if query
// inspection is added later, it should fail and be inverted.
func TestSQLiDoesNotInspectQueryString(t *testing.T) {
	out := captureAlerts(t, func() {
		probe(sqliHandler(), http.MethodGet, "/api/products?id=1'%20OR%201=1--", "203.0.113.5", "")
	})

	if strings.Contains(out, sqliMarker) {
		t.Fatal("query strings are now inspected -- update this test, the gap is closed")
	}
}
