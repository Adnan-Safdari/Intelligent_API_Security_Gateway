...
### `consumer.py`
Reads the `attack_events` stream using a Redis **consumer group**, which is Redis
...
### `ingest.py`
Parses the gateway's printed `SECURITY ALERT` blocks and writes them into Redis:
